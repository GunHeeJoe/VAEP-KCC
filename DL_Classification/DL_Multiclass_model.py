...
class CustomDataset():  
    ...
    def get_xy(self, train_valid_test):        
        if train_valid_test == 1:
            ...
            X = pd.read_csv('./soccer_multiclass_data/X_train',index_col=0)
            Y = pd.read_csv("./soccer_multiclass_data/Y_train",index_col=0)   
            ...
            X = pd.read_csv('./soccer_multiclass_data/X_valid',index_col=0)
            Y = pd.read_csv("./soccer_multiclass_data/Y_valid",index_col=0)           
        elif train_valid_test == 3:
            print("\nmaking test_dataset")
            X = pd.read_csv('./soccer_multiclass_data/X_test',index_col=0)
            Y = pd.read_csv("./soccer_multiclass_data/Y_test",index_col=0)           
        else:
            print("error")
                
        return X, Y
    
    def __init__(self, train_valid_test):
        super(CustomDataset, self).__init__()

        X, Y = self.get_xy(train_valid_test)
...
def valid_evaluation(model, data_loader, criterion, epoch, device, log_interval=5):
    ...
    print("score/concede ROC AUC", roc_auc_result)
    print("score/concede brier", brier_result)
...
def main(model_name, epoch, learning_rate, batch_size, weight_decay, device, save_dir):
    ...
    # train 80%->1, test 20%->0
    train_dataset = CustomDataset(1) 
    valid_dataset = CustomDataset(2) 
    ...
    for epoch_i in range(epoch):
        ...
        auc1, auc2 = valid_evaluation(model, valid_data_loader, criterion, epoch_i, device)

        print('epoch : ', epoch_i)
        if not early_stopper.is_continuable(model, auc1) and not early_stopper.is_continuable(model, auc2):
    ...
    probability.to_csv("./result/Dcn_multiclass_result")
